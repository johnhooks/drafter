...
describe('display preference and expression focus', () => {
  it('defaults to grid and constraints on, handles and sizes off', () => {
    expect(A.initialState().display).toEqual({ grid: true, constraints: true, handles: false, sizes: false })
  ...
  it('patches one toggle at a time and never touches history', () => {
    ...
    expect(s.display).toEqual({ grid: true, constraints: true, handles: false, sizes: true })
    s = A.setDisplay(s, { grid: false })
    expect(s.display).toEqual({ grid: false, constraints: true, handles: false, sizes: true })
