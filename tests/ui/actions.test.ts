...
function cube() {
  ...
  s = A.addRectangle(s, 's1', IN(0), IN(24), IN(0), IN(24), R, 'rect')
...
describe('rectangles', () => {
  const rect = (s: A.State) => (s.doc.features[0] as SketchFeature).rects
  it('addRectangle records the rectangle with a running handle', () => {
    let s = A.addSketch(A.initialState(), DEFAULT_PLANE, 's1')
    s = A.addRectangle(s, 's1', 0, IN(24), 0, IN(16), R, 'rect1')
    expect(rect(s)).toEqual([{ id: 'rect1', handle: 'r1', lines: ['r_l', 'r_b', 'r_r', 'r_t'] }])
    s = A.addRectangle(s, 's1', IN(30), IN(40), 0, IN(10), undefined, 'rect2')
    expect(rect(s).map((r) => r.handle)).toEqual(['r1', 'r2'])
    s = A.explodeRectangle(s, 's1', 'rect1')
    expect(rect(s).map((r) => r.handle)).toEqual(['r2'])
    expect(lines(s)).toHaveLength(8)
    s = A.addRectangle(s, 's1', IN(50), IN(60), 0, IN(10), undefined, 'rect3')
    expect(rect(s).map((r) => r.handle)).toEqual(['r2', 'r3'])
  })
  it('rectangle names resolve through the member lines', () => {
    let s = cube()
    s = A.addLine(s, 's1', { dir: 'v', at: IN(30), from: 0, to: IN(4), attach: false }, 'x')
    s = A.setLineSlot(s, 's1', 'x', 'at', 'r1.right + 1')
    expect(s.eval.errors).toEqual([])
    const sr = s.eval.results.get('s1')
    expect(sr?.kind === 'sketch' && sr.lines.get('x')!.at).toBe(IN(25))
    s = A.setLineSlot(s, 's1', 'x', 'at', 'r1.width / 2')
    expect(sr?.kind === 'sketch' && (s.eval.results.get('s1') as typeof sr).lines.get('x')!.at).toBe(IN(12))
    s = A.setLineSlot(s, 's1', 'x', 'at', 'r1.middle')
    expect(s.eval.errors[0]!.message).toMatch(/r1 is a rectangle and has left, right, bottom, top, width, height, umid, vmid; it has no middle/)
  })
  it('the form writes member lines; width and height write the far line as near plus value', () => {
    let s = cube()
    s = A.setRectSlot(s, 's1', 'rect', 'left', IN(4))
    expect(lines(s).find((l) => l.id === 'r_l')!.at).toBe(IN(4))
    expect(A.regionsOf(s, 's1')[0]!.bounds).toMatchObject({ u0: IN(4), u1: IN(24) })
    s = A.setRectSlot(s, 's1', 'rect', 'width', IN(20))
    expect(lines(s).find((l) => l.id === 'r_r')!.at).toBe(IN(24))
    s = A.addParam(s, 'ply', '3/4')
    s = A.setRectSlot(s, 's1', 'rect', 'width', 'ply * 2')
    expect(lines(s).find((l) => l.id === 'r_r')!.at).toBe('l1.at + (ply * 2)')
    expect(A.regionsOf(s, 's1')[0]!.bounds.u1).toBe(IN(4) + 24)
    s = A.setRectSlot(s, 's1', 'rect', 'width', IN(10))
    expect(s.notices.at(-1)!.text).toMatch(/l3 is fixed by l1.at \+ \(ply \* 2\)/)
    s = A.setRectSlot(s, 's1', 'rect', 'height', 'ply')
    expect(lines(s).find((l) => l.id === 'r_t')!.at).toBe('l2.at + (ply)')
  })
  it('group needs four attached lines, two each way, not already grouped', () => {
    let s = A.addSketch(A.initialState(), DEFAULT_PLANE, 's1')
    s = A.addLine(s, 's1', { dir: 'h', at: 0, from: 0, to: IN(10) }, 'a')
    s = A.addLine(s, 's1', { dir: 'v', at: IN(10), from: 0, to: IN(5) }, 'b')
    s = A.addLine(s, 's1', { dir: 'h', at: IN(5), from: IN(10), to: 0 }, 'c')
    s = A.groupRectangle(s, 's1', ['a', 'b', 'c'])
    expect(s.notices.at(-1)!.text).toMatch(/Four lines are needed .*3 selected/)
    s = A.addLine(s, 's1', { dir: 'v', at: 0, from: IN(5), to: 0 }, 'd')
    s = A.groupRectangle(s, 's1', ['a', 'b', 'c', 'd'], 'g')
    expect(rect(s)).toEqual([{ id: 'g', handle: 'r1', lines: ['d', 'a', 'b', 'c'] }])
    s = A.groupRectangle(s, 's1', ['a', 'b', 'c', 'd'])
    expect(s.notices.at(-1)!.text).toMatch(/already belongs to a rectangle/)
    // an unattached corner is refused by name
    s = A.explodeRectangle(s, 's1', 'g')
    s = A.setLineSlot(s, 's1', 'a', 'max', IN(10))
    s = A.groupRectangle(s, 's1', ['a', 'b', 'c', 'd'])
    expect(s.notices.at(-1)!.text).toMatch(/l1's right end is not attached to l2/)
    expect(rect(s)).toEqual([])
  })
  it('deleting a member dissolves the rectangle and rewrites references to it', () => {
    let s = cube()
    s = A.addLine(s, 's1', { dir: 'v', at: IN(30), from: 0, to: IN(4), attach: false }, 'x')
    s = A.setLineSlot(s, 's1', 'x', 'at', 'r1.right + 1')
    s = A.removeLines(s, 's1', ['r_t'])
    expect(rect(s)).toEqual([])
    expect(lines(s)).toHaveLength(4)
    expect(lines(s).find((l) => l.id === 'x')!.at).toBe('l3.at + 1')
    // the open loop fails the extrude, not the sketch
    expect(s.eval.errors.map((e) => e.featureId)).toEqual(['e1'])
  })
  it('explode rewrites references to the rectangle over its lines', () => {
    let s = cube()
    s = A.addLine(s, 's1', { dir: 'v', at: IN(30), from: 0, to: IN(4), attach: false }, 'x')
    s = A.setLineSlot(s, 's1', 'x', 'at', 'r1.right + 1')
    s = A.setLineSlot(s, 's1', 'x', 'size', 'r1.width / 2')
    s = A.explodeRectangle(s, 's1', 'rect')
    const x = lines(s).find((l) => l.id === 'x')!
    expect(x.at).toBe('l3.at + 1')
    expect(x.run.size).toBe('(l3.at - l1.at) / 2')
    expect(s.eval.errors).toEqual([])
  })
})
