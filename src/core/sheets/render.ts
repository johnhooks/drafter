...
import { THIN_STROKE_WIDTH, VISIBLE_EDGE_WIDTH } from './style'
...
export function renderSheet(sheet: Sheet, projection: Projection, doc: Pick<Document, 'title'>, index: number, total: number, date: string, image?: string): string {
  ...
  const lines = sheet.view === 'isometric'
    ...
    return `<line x1="${firstU}" y1="${firstV}" x2="${secondU}" y2="${secondV}" stroke-width="${segment.visible ? VISIBLE_EDGE_WIDTH : THIN_STROKE_WIDTH}"${segment.visible ? '' : ' stroke-dasharray="0.08 0.04"'}/>`
  }).join('')
