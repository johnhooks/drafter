...
On such a plane, u runs right to left on screen. Dragging to the right decreases u. The lines' coordinates are still correct in the model; only the view is mirrored, exactly as if you walked round the part.
...
The width and height labels and the properties panel are unaffected: a region's width is always its u extent, its height the v extent, regardless of which way the view reads.
