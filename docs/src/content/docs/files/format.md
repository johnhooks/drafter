    ...
    "features": [
      {
        ...
        "rects": [{ "id": "r_b0", "handle": "r1", "lines": ["l_b1", "l_b2", "l_b3", "l_b4"] }]
      },
      { "kind": "extrude", "id": "e_c3", "name": "Extrude 1", "sketchId": "s_a1", "regions": [{ "vertical": "l_b1", "horizontal": "l_b2" }], "distance": 384, "op": "new" },
      {
        ...
        "rects": [{ "id": "r_e0", "handle": "r1", "lines": ["l_e1", "l_e2", "l_e3", "l_e4"] }],
...
- **lines** each have a `dir` of `h` or `v`, an `at` position on the axis the line crosses, and a `run` with exactly two of `min`, `max`, `size` along the axis it runs on. A number is sixteenths; a string is an expression. A run end written as `l2.at` is a corner attached to that line. Optional `construction: true` keeps a line out of every region, and optional `layout` holds constraint and label placements per slot.
- **rects** are the rectangles kept as named groups: a handle and the ids of the left, bottom, right, and top member lines. Expressions such as `r1.width` read through them. A line belongs to at most one rectangle.
