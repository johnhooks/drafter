...
In Sheets mode, **Export sheet as SVG** downloads the selected drawing page in black on white, sized in inches and including its title block, dimensions, and notes. Detached dimensions retain their orange warning colour. **Print Sheet** and **Print All** open the browser print dialog. See [Drawing sheets](/drawings/sheets/) for annotations, scale, and page-orientation settings.
