...
Exploding a rectangle SHALL remove its record and leave its lines and their attachments unchanged. Grouping SHALL turn four selected lines into a rectangle when they are two vertical and two horizontal lines and each run end of each is attached to a perpendicular one of the four; otherwise it SHALL be refused with a message saying what is missing.
...
Deleting a member line SHALL remove the rectangle record and leave the other three as loose lines, with the deletion rule for lines in `sketch` applied to their attached ends.
